...
def ask_claude(prompt):
    ...
    model's final text (search happens inside this one call), or None if
    the call failed -- callers should skip that file rather than crash.
    """
    try:
        ...
        # A bad/missing API key will fail identically on every single call --
        # no point burning through 20+ more calls to prove that 20+ more
        # times. Fail loudly and immediately instead.
        print("FATAL: Anthropic API authentication failed. Check that the "
              "ANTHROPIC_API_KEY repository secret is set correctly.", file=sys.stderr)
        print(f"  ({e})", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        # Anything else (rate limit, transient network issue, etc.) -- log
        # it and let the caller skip this one file rather than taking down
        # the whole run.
        print(f"WARNING: Claude API call failed, skipping this file: {e}", file=sys.stderr)
        return None
...
"""
    ...
    if reply is None:
        return  # already logged inside ask_claude; skip this file, don't crash the run
    try:
...
def main():
    ...
    # Leadership (CM/PM/ministers) runs every day -- cabinet reshuffles are
    # comparatively time-sensitive, and these checks are cheap (small prompts,
    # few searches).
    for f in LEADERSHIP_FILES:
        check_leadership(f)

    # MLA/MP rosters change rarely (elections are once every 5 years; the only
    # triggers between elections are by-elections, deaths, or defections), but
    # each check scans hundreds of constituencies with web search enabled --
    # by far the most expensive part of this pipeline. Running that nightly
    # is overkill for how infrequently it finds anything, so it's gated to
    # run on Sundays only (UTC), unless FORCE_MLA_CHECK=true is set (e.g. for
    # a manual "Run workflow" trigger while testing).
    run_mla_check = (
        datetime.now(timezone.utc).weekday() == 6  # Sunday
        or os.environ.get("FORCE_MLA_CHECK", "").lower() == "true"
    )
    if run_mla_check:
        for f in MLA_FILES:
            check_mlas(f)
        ...
        changes_log.append("- MLA/MP check skipped today (runs weekly, on Sundays UTC). "
                            "Set FORCE_MLA_CHECK=true to run it on demand.")
